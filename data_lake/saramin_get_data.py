...
def fetch_saramin_job_data(data_interval_end, **kwargs) -> str:
    """
    API 호출하여 데이터를 json 파일로 저장
    """

    # 문자열을 datetime 객체로 변환
    data_interval_end_dt = datetime.strptime(data_interval_end, "%Y-%m-%d %H:%M:%S%z")

    # 한국 시간으로 변환
    data_interval_end_kst = data_interval_end_dt.astimezone(KST)
